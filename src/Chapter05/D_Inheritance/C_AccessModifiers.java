...
// 1. private : 접근 제한자가 지정된 멤버(필드, 메소드 등)는 동일한 클래스 내에서만 접근 가능
// 			 클래스 외부에서는 접근할 수 없다. 가장 제한적인 접근 제한자
...
// 3. protected
// : protected로 지정된 멤버는 동일한 패키지 내의 모든 클래스와 다른 패키지의 하위 클래스에서 접근 할 수 있다.

class MyClass3 {
	
	// 같은 패키지 내의 다른 클래스 및 다른 패키지의 하위 클래스에서 접근 가능
	// 부모 클래스와 같은 패키지의 다른 클래스,
	// 다른 패키지의 자식 클래스에서 접근 가능(상속관계에서 주로 사용)
