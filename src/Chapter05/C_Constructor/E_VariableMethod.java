 ...
 * 지역변수 : 메서드 내에서 선언된 변수(인스턴스 변수와 클래스 변수의 경우 전역변수)
...
public class E_VariableMethod {
	...
	int size = MoniotrSize.HEIGHT * MoniotrSize.WIDTH;
	...
	public static void main(String[] args) {
		...
		int num1 = 10; // 메인 메소드 블록 내에서 선언되었음으로 메인 메소드 전역에서 사용 가능, 메인메소드 전역변수
		
		if (num1 > 10) {
			int num2 = 10; // if 블록 내에서 선언되었으므로 if 블록 내에서만 사용 가능, if 블록 지역변수
			if(num2 > 20) {
				int num3 =30; // if 블록 내에서 선언되었으므로 if 블록 내에서만 사용 가능, if 블록 지역변수
			}
		}
			...
			int num2 = 9; 
			// 이전의 num2변수와는 다른 변수
			// else-if 블록 내에서 선언되었으므로 else-if 블록 내에서만 사용 가능, else-if 블록 지역변수
		}
